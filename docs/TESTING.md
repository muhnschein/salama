...
- [ ] Dragging the grid downwards past its top brings the page back the same way; a short pull springs back to the grid. The drag can start on a preview, between previews, or on the head row; the row of groups along the foot is too near the edge of the screen for a pull from there to go far enough.
...
- [ ] The grid's head carries the new-tab button in its left corner and search in its right; both, and the first row of cells with its close buttons, are clear of the screen cutout. The strip of tab groups runs along the foot, centred on the screen, with the edit button in its left corner.
- [ ] The strip reads "N tabs" at first, the current one underlined in the highlight colour and its name highlighted, the names close together in small type and the row centred while it fits. The edit button in the strip's left corner opens the list of groups: the "New tab group" row under the last group makes one, tap it to make it current -- the grid shows it empty, and the new-tab button in the head opens a tab in it. There is no pulley. A group's menu renames and deletes; the "N tabs" group has neither.
...
- [ ] The search button in the head's right corner lists every open tab under its group's heading; typing filters by title or address without the list jumping or the keyboard closing as results come and go; tapping a result brings that tab to the front, puts the grid away and moves the strip to its group.
- [ ] Hold a preview until it lifts and carry it down onto another group's name in the strip: the name lights while the preview is over it, the current group's never does, and no preview trades places while the finger is over the strip. Let go there: the tab is in that group and keeps its page (no reload); the grid stays on its own group, one preview fewer -- unless it was the tab in front, when the grid moves to the new group with it. Carried off the strip again, nothing stays lit.
...
- [ ] Holding the new-tab button in the head row brings a panel up from the foot of the grid listing the tabs closed lately, newest first, with title and address; tapping one opens it again with its title and returns to it, and it leaves the list. Tapping outside the panel puts it away. The list survives a restart.
...
- [ ] The grid's two rows are drawn over the cells rather than among them: the head carries new tab and search, the foot the groups, and the first and last rows of cells can be scrolled clear of both.
- [ ] The new-tab button in the head row opens a new tab and returns to it.
...
- [ ] The menu button brings a sheet up from under the bar: three rows under the headings Tabs, This page and Browser, every entry a theme icon (none blank) with its name under it. Each does what it says and the sheet goes; a tap outside puts it away. With the address field up, the menu ends editing and the keyboard goes.
