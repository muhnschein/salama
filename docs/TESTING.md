...
- [ ] The strip reads "N tabs" and "Private" at first, the current one underlined in the highlight colour and its name highlighted, the names close together in small type and the row centred while it fits. The edit button in the strip's left corner opens the list of groups: pull down to make one, tap it to make it current -- the grid shows it empty, and the button in the foot opens a tab in it. Its menu renames and deletes; the last ordinary group and the private group have neither.
...
- [ ] Menu > New private tab opens in the Private group and the strip moves there; a tab opened from the foot button while Private is current is private too; Menu > Move tab to group is disabled for it, and the picker never offers Private for an ordinary tab. Kill the app, relaunch: the private tabs are still there, still private, and their pages still leave no history.
