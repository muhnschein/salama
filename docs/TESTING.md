...
- [ ] The whole bar is opaque and the page ends above it: scroll to the foot of a long page -- a site footer, a cookie banner -- and every control there can be tapped without the bar in the way. Slim, the bar is a faint wash the page shows through, and the host stays readable over a white page; a control under the slim bar, beside its handle, still takes the tap.
- [ ] Scrolling down a page slims the bar to the handle and the host alone, at a smaller size, with its background fading away as it goes; scrolling back up brings both back. The change is one smooth movement -- the bar's height, the controls fading, the host's size -- not a jump. Tapping the address, and loading a new page, both bring the whole bar back. The page does not reflow as the bar changes height.
