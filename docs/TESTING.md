...
- [ ] The grid's head carries a "Search tabs" field across it, its words from the left edge; it and the first row of cells with its close buttons are clear of the screen cutout. The foot carries the new-tab button in its left corner and the edit button in its right, and the strip of tab groups between them, centred on the screen.
- [ ] The strip reads "N tabs" at first, the current one underlined in the highlight colour and its name highlighted, the names close together in small type and the row centred while it fits. The edit button in the foot's right corner opens the list of groups: the "New tab group" row under the last group makes one, tap it to make it current -- the grid shows it empty, and the new-tab button in the foot's left corner opens a tab in it. There is no pulley. A group's menu renames and deletes; the "N tabs" group has neither.
...
- [ ] Bookmark the page from the menu (the star fills and lights); it appears in Bookmarks; edit and remove work.
