...
3. Tag: `git tag -s vX.Y.Z -m "salama X.Y.Z"` and push the tag.
4. Build: the `rpm` workflow stamps `X.Y.Z` and `1.<run number>` into
   `rpm/harbour-salama.spec`, builds the `aarch64` RPM and runs Jolla's validator on it.
   Locally the same is
   `sed -i 's/^Version:.*/Version:    X.Y.Z/' rpm/harbour-salama.spec && sfdk build`.
   A `build-*` tag builds any commit the same way without being a release.
5. Validate: `sfdk check -s harbour RPMS/harbour-salama-X.Y.Z-1.aarch64.rpm` must pass,
