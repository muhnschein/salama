...
D-Bus. Sailjail lets **only sailfish-browser's own profile** make that call
(`permissions/sailfish-browser.profile` in sailjail-permissions); no permission a Harbour
application may ask for grants it. The list would not hold this browser's downloads in any
case: its entries are made by sailfish-browser itself (`apps/core/downloadmanager.cpp`),
from the engine's `embed:download` notifications, and sailfish-components-webview makes
none for the other applications that embed the engine. SCOPE.md's "Downloads via platform
...
`WebEngine`, which SCOPE.md §5 keeps to the browsing page and Settings; it is a later
change, not a refusal.
