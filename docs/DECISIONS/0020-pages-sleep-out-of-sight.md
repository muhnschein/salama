...
processor ran as if the page were on the screen. The views were inactive —
`WebView.active` is false out of sight, which stops the engine drawing and lets it slow
a page's timers down — but a page's timers, workers, idle callbacks and the scripts
they run went on, and a busy page has plenty of each.
...
What it does not suspend is a page making a sound. The media element itself plays on in
a frozen page — that is what keeps music going with the screen locked, the subject of
the Gecko patch below — but a player feeds its stream and starts its next track from
the page's own scripts, and those are frozen too. The browser knows a page is playing
from two observer topics: every media decoder reports its metadata (does the stream
have sound) and each change of its play state on `media-decoder-info`, a Sailfish patch
to Gecko (`rpm/0056-Ensure-audio-continues-when-screen-is-locked.patch`), and a WebRTC
call reports its microphone and camera on `webrtc-media-info`.
...
set. Out of sight nothing is drawn, and coming back is a view going active.
...
already asleep, which a frozen page cannot start, is not handled.
