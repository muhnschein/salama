...
### Added
- A tab whose page plays something with sound says so: its preview in the grid carries a pause button and a mute button in its bottom-left corner, and the navigation bar carries the same two left of the host. Pausing from either shows a play button in its place; a tab behind the one in front, which the engine keeps silent, shows play, and playing it brings it to the front. A muted tab stays muted across its pages until it is unmuted or closed. Only media the page's own scripts can reach has the controls: not a player embedded from another site.
