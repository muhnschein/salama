...
- Tab groups, as Safari has them: the grid shows one group at a time, and a strip along its foot names them, centred, the current one underlined and kept in the middle. An edit button in the strip's corner lists the groups to rename and delete them, with a row under the last one that makes a new group; a search button in the head of the grid finds a tab by title or address across every group. Groups survive a restart.
...
- The new-tab button sits in the head of the grid, in its left corner; holding it brings up the tabs closed lately, to open again, and the list survives a restart.
...
- The menu is a sheet of icons that comes up from under the navigation bar, in three rows: a new tab; search on the page, bookmark it, share it, or ask for its desktop version; bookmarks, history, downloads and settings. A tap outside puts it away.
